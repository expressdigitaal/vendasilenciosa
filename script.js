// ============================================
// PROTEÇÃO ANTI-CLONAGEM DO SITE
// ============================================

// 1. Desabilitar clique direito
document.addEventListener('contextmenu', function(e) {
    e.preventDefault();
    alert('⚠️ Conteúdo protegido! A cópia não é permitida.');
    return false;
});

// 2. Desabilitar teclas de atalho (F12, Ctrl+U, Ctrl+S, Ctrl+Shift+I, etc)
document.addEventListener('keydown', function(e) {
    // F12 (DevTools)
    if (e.keyCode === 123) {
        e.preventDefault();
        alert('⚠️ DevTools desabilitado!');
        return false;
    }
    ...
    if (e.ctrlKey && e.keyCode === 83) {
        e.preventDefault();
        alert('⚠️ Não é permitido salvar esta página!');
        return false;
    }
});

// 3. Detectar se DevTools está aberto
(function() {
    const threshold = 160;
    setInterval(function() {
        if (window.outerWidth - window.innerWidth > threshold || 
            window.outerHeight - window.innerHeight > threshold) {
            document.body.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:100vh;background:#000;color:#FFD700;font-size:2rem;text-align:center;padding:20px;">⚠️ DevTools Detectado!<br>Recarregue a página.</div>';
        }
    }, 1000);
})();

// 4. Prevenir embedding em iframe
if (window.top !== window.self) {
    window.top.location = window.self.location;
}

// 5. Desabilitar arrastar imagens
...
// 6. Proteção contra seleção de imagens
...
// 7. Watermark invisível no console
console.log('%c🔒 SITE PROTEGIDO', 'color: #FFD700; font-size: 24px; font-weight: bold; text-shadow: 2px 2px 4px #000;');
console.log('%c⚠️ ATENÇÃO: A clonagem deste site é PROIBIDA!', 'color: #ff0000; font-size: 16px; font-weight: bold;');
console.log('%c© 2025 Venda Silenciosa Start - Todos os direitos reservados.', 'color: #fff; font-size: 12px;');
console.log('%cQualquer violação será processada judicialmente.', 'color: #FFD700; font-size: 12px;');

// 8. Limpar console periodicamente
setInterval(function() {
    console.clear();
    console.log('%c🔒 SITE PROTEGIDO', 'color: #FFD700; font-size: 20px; font-weight: bold;');
}, 3000);
