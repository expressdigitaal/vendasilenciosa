...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // Permitir pular animação clicando em qualquer lugar ou apertando qualquer tecla
    logoIntro.addEventListener('click', skipAnimation);
    ...
    // Esconder a logo inicial após 3.5 segundos (tempo da animação)
    setTimeout(() => {
        if (logoIntro) {
            logoIntro.style.opacity = '0';
            logoIntro.style.transition = 'opacity 0.5s ease';
    ...
    // FALLBACK DE SEGURANÇA: Se algo der errado, força o site a aparecer após 5 segundos
    setTimeout(() => {
        if (logoIntro && !logoIntro.classList.contains('hidden')) {
            console.warn('⚠️ Forçando saída da animação...');
            logoIntro.style.display = 'none';
            logoIntro.classList.add('hidden');
            if (header) {
                header.classList.add('visible');
